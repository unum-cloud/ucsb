...
struct workload_t {
    ...
    float bulk_insert_proportion = 0;
    ...
    size_t bulk_insert_min_length = 0;
    size_t bulk_insert_max_length = 0;
    distribution_kind_t bulk_insert_length_dist = distribution_kind_t::uniform_k;
...
bool load(fs::path const& path, workloads_t& workloads) {
    ...
    for (auto j_workload = j_workloads.begin(); j_workload != j_workloads.end(); ++j_workload) {
        ...
        workload.bulk_insert_proportion = (*j_workload).value("bulk_insert_proportion", 0.0);
        ...
        workload.bulk_insert_min_length = (*j_workload).value("bulk_insert_min_length", 256);
        workload.bulk_insert_max_length = (*j_workload).value("bulk_insert_max_length", 256);
        workload.bulk_insert_length_dist =
            parse_distribution((*j_workload).value("bulk_insert_length_dist", "uniform"));
        if (workload.bulk_insert_length_dist == distribution_kind_t::unknown_k) {
            workloads.clear();
            return false;
        }
