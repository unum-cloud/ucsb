...
struct rocksdb_t : public ucsb::db_t {
  public:
    inline rocksdb_t() : db_(nullptr), slice_buffer_(100) {}
  ...
  private:
    inline rocksdb::Slice build_slice(key_t key);
...
operation_result_t rocksdb_t::insert(key_t key, value_spanc_t value) {
    ...
    rocksdb::Slice slice = build_slice(key);
...
rocksdb::Slice rocksdb_t::build_slice(key_t key) {
    char str_format[] = "%d";
    int len = std::snprintf(slice_buffer_.data(), slice_buffer_.size(), str_format, key);
    return rocksdb::Slice(slice_buffer_.data(), len);
}
