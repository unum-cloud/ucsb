...
operation_result_t lmdb_t::batch_insert(keys_spanc_t keys, values_spanc_t values, value_lengths_spanc_t sizes) {
    ...
    for (size_t idx = 0; idx < keys.size(); ++idx) {
        ...
        key_slice.mv_size = sizeof(key_t);
        val_slice.mv_data = const_cast<void*>(reinterpret_cast<void const*>(&values[idx]));
...
operation_result_t lmdb_t::bulk_insert(keys_spanc_t keys, values_spanc_t values, value_lengths_spanc_t sizes) {
