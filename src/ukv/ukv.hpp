...
private:
  inline value_view_t make_value(std::byte const *ptr, size_t length) {
    return {reinterpret_cast<ukv_bytes_cptr_t>(ptr),
            static_cast<ukv_length_t>(length)};
  }
...
operation_result_t ukv_t::upsert(key_t key, value_spanc_t value) {
  ...
  return {size_t(status),
          status ? operation_status_t::ok_k : operation_status_t::error_k};
}
...
operation_result_t ukv_t::batch_upsert(keys_spanc_t keys, values_spanc_t values,
  ...
  return {status ? keys.size() : 0,
          status ? operation_status_t::ok_k : operation_status_t::error_k};
}

operation_result_t ukv_t::batch_read(keys_spanc_t keys,
...
operation_result_t ukv_t::bulk_load(keys_spanc_t keys, values_spanc_t values,
                                    value_lengths_spanc_t sizes) {
  return batch_upsert(keys, values, sizes);
}

operation_result_t ukv_t::range_select(key_t key, size_t length,
                                       values_span_t values) const {
  status_t status;
...
operation_result_t ukv_t::scan(key_t key, size_t length,
